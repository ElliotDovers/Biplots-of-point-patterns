...
pca <- prcomp(iris[ , -5], scale. = F) # prcomp auto centers the data
# pca <- prcomp(iris[ , -5], scale. = T) # prcomp auto centers the data
Sigma <- cov(iris[,-5])

# set the biplot components
G <- pca$x[,1:2]
H <- pca$rotation[,1:2] %*% diag(pca$sdev[1:2])

# get the estimated covariance
Sigma_hat <- cov2cor(H %*% t(H))
# set up nicer names
...
Sigma_hat[3,1] <- as.numeric(substring(as.character(Sigma_hat[3,1]), 1, 4))
Sigma_hat[4,1] <- as.numeric(substring(as.character(Sigma_hat[4,1]), 1, 4))
Sigma_hat[4,3] <- as.numeric(substring(as.character(Sigma_hat[4,3]), 1, 4))

# rotate the data for aesthetic reasons
theta <- (5/6) * pi
...
{
  par(mfrow = c(2,2), mar = c(2.1, 2.1, 3.1, 1))
  ...
  #      ylim = range(c(G[,2], loads[,2])), xlab = "Principal Component 1", ylab = "Principal Component 2")
  plot(G, col = iris$Species, asp = 1, pch = as.numeric(iris$Species), cex = 0.75,
       xlab = "Principal Component 1", ylab = "Principal Component 2")
  mtext(expression(bold("A: Iris Dataset Biplot")), side = 3, line = 1.4, adj = 0, at = -5.45)
  mtext(expression(bold(paste(X%~~%U, "\u039b"^T, " (points U, lines \u039b)"))), side = 3, line = 0.15, adj = 0, cex = 0.8, at = -5.45)
  arrows(x0 = rep(0, nrow(loads)), y0 = rep(0, nrow(loads)), x1 = loads[,1], y1 = loads[,2], length = 0.05, angle = 30, lwd = 0.5)
  text(x = loads[,1] + c(0, 0, 0.15, 0), y = loads[,2] + c(0.15, 0.15, 0.15, -0.25), labels = nice.names, cex = 0.75)
  
  par(mar = c(2.1, 2.1, 2.1, 1))
  corrplot(Sigma_hat, type = "lower", method = "square", tl.srt = 45, diag = F, tl.col = "black", tl.cex = 0.75, addCoef.col = rep("black", 6), cl.pos = "n", col = "white", mar = c(0, 0, 2.2, 0), is.corr = T)
  mtext(expression(bold(paste("B: Estimated Correlation, \u039b\u039b"^T~" (scaled)"))), side = 3, line = 0.4, adj = 0)
...
corrplot(Sigma_hat, type = "lower", method = "square", tl.srt = 45, diag = F, tl.col = "black", tl.cex = 0.75, addCoef.col = rep("black", 6), cl.pos = "n", number.cex = 2.5, col = "white", mar = c(0, 0, 0, 0), is.corr = F)
