...
des.mat <- as.matrix(cbind(mod_str$data.tmb$X, newZ))
...
for (i in ks) {
  ...
  # # Attempt to calculate the EDF - this is computaitonally expensive
...
res <- data.frame(
  set_k = ks, fitted_k = actual_k, mae, ll,# edf, aic, bic, 
