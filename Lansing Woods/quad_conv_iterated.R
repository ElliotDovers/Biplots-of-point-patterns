...
# set the scope for q
tmp.qs <- c(1000, 5000, seq(10000, 100000, by = 30000), 20000, 30000, 50000, 60000, 80000, 90000)
...
# create the data
set.seed(job)
quad <- data.frame(x = runif(n_q[job]), y = runif(n_q[job]), pt = 0, wt = lansing$window$units$multiplier^2 / n_q[job])
set.seed(NULL)
tmp <- quad[rep(seq_len(n_q[job]), length(levels(pp$tree))), ]
tmp$tree <- rep(levels(pp$tree), each = n_q[job]) # assign the tree classes
dat <- rbind(pp, tmp)
rm(quad, tmp)
...
res <- data.frame(
  q = n_q[job], rep = rep.idx[job], ll0 = logLik(tmp.m0), ll = logLik(tmp.m)
)
